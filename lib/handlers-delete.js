...
handlers.handleMethods = function(acceptableMethods, data, endpoints, callback, methods) {
  ...
  if (acceptableMethods.indexOf(data.method) > -1) {
    ...
    if (typeof endpointObject == 'object') {
      ...
      // data.auth contains authentication pairs including client and
      // endpointObject.paramsObject should also contains client key
      // Both has to have same value otherwise 401 code (not authorized)
      // will be returnd
      
      if (data.auth.client != endpointObject.paramsObject.client)  {
        callback(401, { 'error': "Not authorized to access other client's data"})
          return 
      }
...
// For Express routes
handlers.authenticateToken = function(req, res, next) {
...
// For MY router routes
// handlers.authenticate = function(data, callback) {
...
//         if (err) {
//           callback(null, { 'code': 403, 'status': 'Access forbindden. Token is invalid.'});
//         }

//         callback(tokenPayload, null);

//       })

//     } catch(error) {
//       console.error('Some error occured during authorize', error);
//       callback(null, { 'code': 403, 'status': 'Some error occured during authorization'});
//     }
//   }
// }

handlers.authException = function(path) {
  const exceptions = ['tokens/createToken'];
...
handlers.get = function(endpoint, callback) {
  ...
  // Query database using provided sql query and parameters 
    ...
    console.log('-------------------------------------------------------------')
    console.log('endpoint.sqlQuery.......:', endpoint.sqlQuery)
    console.log('endpoint.paramsObject...:', endpoint.paramsObject)
    console.log('-------------------------------------------------------------')
      ...
      if (error) {
            ...
            if (results[0].length === 0) {
              ...
              if (typeof endpoint.before200 === 'function') { 
                console.log('before calling translate', results[0])
                results[0] = endpoint.before200(results[0], endpoint.paramsObject); 
                console.log('after calling translate', results[0])
              } 
              // Use callback to return records found in database 
              callback(200,results[0]);
            }
